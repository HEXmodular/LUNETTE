...
esp_err_t api_registry_init(httpd_handle_t server)
{
    ...
    // Register global OPTIONS handler for all API endpoints
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TAG, "Failed to register global OPTIONS handler");
        return err;
    }
    ESP_LOGI(TAG, "Registered global OPTIONS handler for /api/*");
